<script setup>
  ...
  const initTabulator = () => {
    tabulator.value = new Tabulator(tableRef.value, {
      ajaxURL: `${import.meta.env.VITE_API_BASE}currencies`,
      ajaxConfig:{
        method:"GET",
        headers: {
...
<template>
  <div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    ...
    <div class="flex w-full mt-4 sm:w-auto sm:mt-0">
      <router-link class="mr-2 shadow-md btn btn-primary" :to="`/add-pipeline`">{{ $t('add_currencies.btn_add_new_currency') }}</router-link>
