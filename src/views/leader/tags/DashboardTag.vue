<script setup>
  ...
  const initTabulator = () => {
    tabulator.value = new Tabulator(tableRef.value, {
      ajaxURL: "https://api.leader.arkanmedia.com/api/tags/list/all",
      ajaxConfig:{
        method:"GET",
        headers: {
  ...
  const initTabulator = () => {
    tabulator.value = new Tabulator(tableRef.value, {
      ...
      ajaxURL: "https://api.leader.arkanmedia.com/api/tags/list",
      ajaxConfig:{
        method:"GET",
        headers: {
            "Authorization": `Bearer ${sessionStorage.getItem("TOKEN")}`,
...
<div class="flex flex-col items-center mt-8 intro-y sm:flex-row">
    ...
    <div class="flex w-full mt-4 sm:w-auto sm:mt-0">
      <router-link class="mr-2 shadow-md btn btn-primary" :to="`/add-tag`">{{ $t('add_tags.btn_add_new_tags') }}</router-link>
