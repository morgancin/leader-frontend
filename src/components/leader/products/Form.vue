  ...
  <div class="intro-y col-span-12 lg:col-span-6">
    ...
    <form @submit.prevent="$emit('submit')" autocomplete="on">
        ...
        <div class="border border-slate-200/60 dark:border-darkmode-400 rounded-md p-5">
          ...
          <div class="mt-5">  
            <div class="form-inline items-start flex-col xl:flex-row mt-10">
              ...
              <div class="w-full mt-3 xl:mt-0 flex-1 dark:border-darkmode-400 rounded-md">
      ...
      <div class="flex justify-end flex-col md:flex-row gap-2 mt-5">
        <button type="button" class="w-full md:w-52 py-3 mr-1 btn btn-outline-secondary">{{ $t('add_products.btn_cancel') }}</button>
        <button type="submit" class="w-full md:w-52 py-3 btn btn-primary">{{ $t('add_products.btn_save') }}</button>
      </div>
  ...
  <Modal :show="addComponentModal" @hidden="addComponentModal = false">
    ...
    <ModalBody class="grid grid-cols-12 gap-4 gap-y-3">      
      <div class="col-span-12">
        <label for="pos-form-5" class="form-label">Componente:</label>
        <v-select
              label="name"
              class="form-control" 
      ...
      <div class="col-span-12">
        <label for="pos-form-4" class="form-label">Cantidad:</label>
  ...
  <Modal :show="addPricesListsModal" @hidden="addPricesListsModal = false">
    ...
    <ModalBody class="grid grid-cols-12 gap-4 gap-y-3">      
      ...
      <div class="col-span-5 flex items-center">
        <label for="pos-form-5" class="form-label mb-0">Precio ($):</label>
        <input
            type="number"
            id="pos-form-4"
            placeholder="0"
            class="w-24 text-center form-control ml-auto"
            v-model="add_pricelist_form.price"/>
      </div>
      <div class="col-span-7 flex items-center">
        <label for="pos-form-5" class="form-label mb-0 mr-2">Divisa:</label>        
        <v-select      
            label="code"      
            class="form-control" 
...
const { add_pricelist_car, remove_pricelist_car } = usePricesListsCartStore();
...
const add_priceslists = () => {
  addPricesListsModal.value = true;  
}

const removePriceList = priceslists => {
  remove_pricelist_car(priceslists);
}

const submit_add_priceslists = () => {
  priceslists_modal.value = add_pricelist_form.name;
  //console.log(priceslists_modal.value);
  add_pricelist_car(
                      priceslists_modal.value,                                                     
                      add_pricelist_form.price,
                      add_pricelist_form.name,
                      add_pricelist_form.currency
                  );
  add_pricelist_form.price='';
  add_pricelist_form.name='';
  add_pricelist_form.currency='';
  addPricesListsModal.value = false;      
}
