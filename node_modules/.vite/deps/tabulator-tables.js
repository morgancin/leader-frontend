...
function xlsx(list2, options, setFileContents) {
  var self = this, sheetName = options.sheetName || "Sheet1", workbook = XLSX.utils.book_new(), tableFeatures = new CoreFeature(this), compression = "compress" in options ? options.compress : true, output;
  workbook.SheetNames = [];
  workbook.Sheets = {};
  ...
  _virtualRenderFill(position, forceMove, offset) {
    ...
    if (rowsCount && Helpers.elVisible(this.elementVertical)) {
      ...
      while ((rowsHeight <= containerHeight + this.vDomWindowBuffer || i < this.vDomWindowMinTotalRows) && this.vDomBottom < rowsCount - 1) {
        var index = this.vDomBottom + 1, row = rows2[index], rowHeight = 0;
        this.styleRow(row, index);
        element.appendChild(row.getElement());
        row.initialize();
...
var Tabulator = class {
  ...
  addRow(data, pos, index) {
    ...
    return this.rowManager.addRows(data, pos, index).then((rows2) => {
