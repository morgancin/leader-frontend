...
import { isString, isArray, isBoolean, isObject, isEmptyObject, isNumber, assign } from '@intlify/shared';
import { createFilter } from '@rollup/pluginutils';
import { checkInstallPackage, generateJSON, generateYAML } from '@intlify/bundle-utils';
...
const unplugin = createUnplugin((options = {}, meta) => {
  ...
  const getAliasName = () => installedPkg === "petite-vue-i18n" && isBoolean(useVueI18nImportName) && useVueI18nImportName ? "vue-i18n" : `${installedPkg}`;
  ...
  return {
    ...
    webpack(compiler) {
      ...
      if (isProduction && runtimeOnly) {
        ...
        compiler.options.resolve.alias["vue-i18n"] = `vue-i18n/dist/vue-i18n.runtime.esm-bundler.js`;
      }
    ...
    async load(id) {
      debug("load", id);
      const { query } = parseVueRequest(id);
      if ([INTLIFY_BUNDLE_IMPORT_DEPRECTED_ID, INTLIFY_BUNDLE_IMPORT_ID].includes(getVirtualId(id, meta.framework)) && include) {
        let resourcePaths = [];
